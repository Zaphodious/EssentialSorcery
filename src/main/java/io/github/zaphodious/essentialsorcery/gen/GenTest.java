...
public class GenTest {
	...
	public static boolean makeAWall(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
		BlockPos workingPos = null;
		for (int j = 0; j < 10; j++) {
			workingPos = pos.up(j);
			for (int i = 0; i < 5; i++) {
				workingPos = (i==0) ? workingPos : workingPos.west();
				IBlockState toPlace = (j>5 || j<3) ? ModBlocks.moon_silver_block.getDefaultState():Blocks.coal_block.getDefaultState();
				world.setBlockState(workingPos,
						toPlace, 3);
	...
	public static boolean makeASimpleBuilding(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, int buildHeight, int buildWidth, int buildLength) {
		...
		/*for (int j = 0; j <= buildHeight; j++) {
			workingPos = pos.up(j+1);
			for (int i = 0; i <= buildWidth; i++) {
				workingPos = (i==0) ? workingPos : workingPos.west();
				IBlockState toPlace = Blocks.cobblestone.getDefaultState();
				
				for (int k = 0; k <= buildLength; k++) {
					if (i != 0 && i != buildWidth && j != buildHeight && k != buildLength) {
						toPlace = Blocks.air.getDefaultState();
					}
					workingPos = (k==0) ? workingPos:workingPos.north();
					world.setBlockState(workingPos, toPlace, 3);
	...
	public static boolean genTestStructure(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
